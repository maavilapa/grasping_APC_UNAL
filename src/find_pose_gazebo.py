...
def predict(depth, process_depth=True, crop_size=crop_size, out_size=crop_size, depth_nan_mask=None, crop_y_offset=0, filters=(4.0, 2.0, 2.0)):
    ...
    width_out = pred_out[3].cpu().numpy().squeeze() * 80.0  # Scaled 0-150:0-1
...
def rvalues(grasp, depth, Dx, Dy):
    ...
    x = (pxn-cx)*0.65/(290)
    y = (pyn-cy)*0.43/(190)
    #medir tamano real del canasto
    #calcular bien el ancho
    #ubicar un objeto justo debajo de la camara
    z = point_depth
...
def find_pose():
    ...
    #    rgbo = rospy.wait_for_message('/camera/rgb/image_color', Image)
        ...
        #print(xx)
        iy, ix, Dy, Dx, widthbinx, widthbiny, iy1, ix1, Dy1, Dx1 =[110, 208, 180, 286, 286, 180, 0, 0, 180, 286]
